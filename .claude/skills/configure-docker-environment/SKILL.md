...
- The build context ships only a prebuilt jar, never source — `.dockerignore` excludes everything else. The jar must exist at `build/docker/modusplant-backend.jar` before `docker compose build` (the same artifact path the CI/CD pipeline produces).
