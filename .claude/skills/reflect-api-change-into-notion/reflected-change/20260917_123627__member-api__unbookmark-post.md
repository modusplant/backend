...
Target Notion Page: 게시글 북마크 취소 API (best match from `MemberRestController.cancelCommunicationPostBookmark`'s `@Operation` summary — confirm exact title via `notion-search` before editing)
...
| 이름 | 타입 | 설명 |
|---|---|---|
| postUlid | String | 북마크를 취소할 게시글의 식별자 |
