...
import kr.modusplant.domains.post.framework.outbound.redis.PostRecentlyViewRedisRepository;
import kr.modusplant.domains.post.framework.outbound.redis.PostViewCountRedisRepository;
...
public class PostRepositoryJpaAdapter implements PostRepository {
    ...
    public void update(Post post) {
        ...
        Long viewCount = postViewCountRedisRepository.read(post.getPostId());
        if (viewCount != null) postEntity.updateViewCount(viewCount);
        ...
        postEntity.updateEditedAt(now);
        if (!postEntity.getIsPublished()) {
            postEntity.updatePublishedAt(post.getStatus().isPublished() ? now : null);
