...
public class Email {
    ...
    public static void validateSource(String email) {
        if (email == null || email.isBlank()) { throw new EmptyValueException(EmailIdentityErrorCode.EMPTY_EMAIL); }
        if (!email.matches(Regex.REGEX_EMAIL)) {
            throw new InvalidValueException(EmailIdentityErrorCode.INVALID_EMAIL);
        }
    }
