...
public class SearchPostHistoryRedisRepository implements SearchPostHistoryRepository {
    ...
    public void saveSearchKeyword(SearchKeyword searchKeyword, UUID memberUuid) {
    ...
    public List<String> getSearchHistory(UUID memberId, int size) {
        String key = generatedKey(memberId);
        ZSetOperations<String, String> zSet = stringRedisTemplate.opsForZSet();
        ...
        // 최신순으로 반환
        Set<String> keywords = zSet.reverseRange(generatedKey(memberId), 0, size-1);
        return keywords != null ? new ArrayList<>(keywords) : Collections.emptyList();
    }

    public void removeSearchKeyword(SearchKeyword searchKeyword, UUID memberUuid) {
        String keyword = searchKeyword.getValue();
    ...
    public void removeAllSearchHistory(UUID memberId) {
        stringRedisTemplate.delete(generatedKey(memberId));
    }
