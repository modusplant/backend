...
import kr.modusplant.domains.communication.domain.validation.CommunicationCategory;
...
@Tag(name = "컨텐츠 1차 항목 API", description = "컨텐츠 1차 항목 도메인을 다루는 API입니다.")
@RestController
@Primary
@RequestMapping("/api/v1/communication/categories/primary")
@RequiredArgsConstructor
@Validated
public class CommPrimaryCategoryController {
    private final CommPrimaryCategoryApplicationService commCategoryApplicationService;

    @Operation(
            summary = "전체 컨텐츠 1차 항목 조회 API",
            description = "전체 컨텐츠 1차 항목의 식별자를 비롯하여 이름, 컨텐츠와 버전 정보를 조회합니다."
    )
    @GetMapping
    ...
    @Operation(
            summary = "UUID로 컨텐츠 1차 항목 조회 API",
            description = "UUID에 맞는 컨텐츠 1차 항목을 조회합니다."
    )
    @GetMapping("/{uuid}")
    public ResponseEntity<DataResponse<?>> getCommCategoryByUuid(
            @Parameter(schema = @Schema(
                    description = "1차 항목의 식별자",
    ...
    @Operation(
            summary = "순서로 컨텐츠 1차 항목 조회 API",
            description = "순서에 맞는 컨텐츠 1차 항목을 조회합니다."
    )
    @GetMapping("/order/{order}")
    public ResponseEntity<DataResponse<?>> getCommCategoryByOrder(
            @Parameter(schema = @Schema(
                    description = "1차 항목이 렌더링되는 순서",
                    minimum = "0",
                    maximum = "100",
                    example = "3")
            )
    ...
    @Operation(
            summary = "1차 항목으로 컨텐츠 1차 항목 조회 API",
            description = "1차 항목에 맞는 컨텐츠 1차 항목을 조회합니다."
    )
    ...
    @Operation(
            summary = "컨텐츠 1차 항목 삽입 API",
            description = "순서, 1차 항목 정보로 컨텐츠 1차 항목을 삽입합니다."
    )
    @PostMapping
    ...
    @Operation(
            summary = "컨텐츠 1차 항목 제거 API",
            description = "UUID로 컨텐츠 1차 항목을 제거합니다."
    )
    @DeleteMapping("/{uuid}")
