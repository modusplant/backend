...
public class ProposalOrBugReportEvent {
    ...
    public static ProposalOrBugReportEvent create(UUID memberId,
        ...
        if (memberId == null) {
            ...
            throw new InvalidValueException(EntityErrorCode.NOT_FOUND_REPORT, "category");
        } else if (content.isEmpty()) {
            throw new InvalidValueException(EntityErrorCode.NOT_FOUND_REPORT, "content");
        } else {
            return new ProposalOrBugReportEvent(memberId, title, category, content, imagePath);
        }
    }
}
