...
import kr.modusplant.framework.jpa.entity.common.util.PostLikeEntityTestUtils;
...
public class PostLikeJpaRepositoryTest implements PostLikeEntityTestUtils {
    @Autowired
    PostLikeJpaRepository commPostLikeRepository;
    ...
    class SetupTest {
        ...
        void likeCommPost_success() {
            ...
            // then
            Optional<PostLikeEntity> commLikeEntity = commPostLikeRepository.findById(new PostLikeCompositeKey(postId, memberId));
    ...
    void findCommPostLikesByMemberId() {
        ...
        commPostLikeRepository.saveAll(List.of(
                PostLikeEntity.of(postIds.get(0), memberId),
                PostLikeEntity.of(postIds.get(1), memberId),
                PostLikeEntity.of(postIds.get(2), memberId)
        ));
        commPostLikeRepository.flush();

        // when
        List<PostLikeEntity> commLikeList = commPostLikeRepository.findByMemberId(memberId);
    ...
    void findCommPostLikesByMemberIdAndPostIds() {
        ...
        // when
        List<PostLikeEntity> commLikeList = commPostLikeRepository.findByMemberIdAndPostIdIn(memberId, postIds);
