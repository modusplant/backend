package kr.modusplant.domains.search.framework.outbound.redis;
...
class SearchPostHistoryRedisRepositoryTest {
    ...
    void testSaveSearchKeyword_givenMemberUuidAndKeyword_willSaveKeyword() {
        ...
        // when
        postSearchHistoryRedisRepository.saveSearchKeyword(testSearchKeyword, MEMBER_BASIC_USER_UUID);

        // then
        verify(zSetOperations).add(
                eq(key),
                eq(TEST_SEARCH_KEYWORD),
    ...
    void testSaveSearchKeyword_givenMemberUuidAndKeyword_willSaveKeywordAndDeleteOldestKeyword() {
        ...
        // then
        verify(zSetOperations).removeRange(key, 0, 0);
        verify(stringRedisTemplate).expire(key, TTL_DAYS, TimeUnit.DAYS);
    }

    @Test
    @DisplayName("회원 id가 null이면 기록하지 않는다")
    void testSaveSearchKeyword_givenNullMemberId_willNotSaveKeyword() {
        postSearchHistoryRedisRepository.saveSearchKeyword(testSearchKeyword, null);
    ...
    void testGetSearchHistory_givenMemberUuidAndSize_willRemoveExpiredAndReturnKeywords() {
        ...
        given(stringRedisTemplate.opsForZSet()).willReturn(zSetOperations);
        given(zSetOperations.reverseRange(key, 0, TEST_SEARCH_POST_SIZE - 1)).willReturn(keywords);

        // when
        List<String> result = postSearchHistoryRedisRepository.getSearchHistory(
                MEMBER_BASIC_USER_UUID, TEST_SEARCH_POST_SIZE);
        ...
        verify(zSetOperations).reverseRange(key, 0, TEST_SEARCH_POST_SIZE - 1);
    ...
    void testRemoveSearchKeyword_givenMemberUuidAndKeyword_willRemoveKeyword() {
        ...
        // when
        postSearchHistoryRedisRepository.removeSearchKeyword(testSearchKeyword, MEMBER_BASIC_USER_UUID);

        // then
        verify(zSetOperations).remove(key, TEST_SEARCH_KEYWORD);
    }
