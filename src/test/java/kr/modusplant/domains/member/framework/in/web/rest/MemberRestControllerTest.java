...
class MemberRestControllerTest implements MemberTestUtils {
    ...
    void testReportProposalOrBug_givenValidRequest_willReturnResponse() throws IOException {
        ...
        // when
        ResponseEntity<DataResponse<Void>> responseEntity = memberRestController.reportProposalOrBug(TEST_REPORT_TITLE, TEST_REPORT_CATEGORY_PROPOSAL, TEST_REPORT_CONTENT, TEST_REPORT_IMAGE, MEMBER_BASIC_USER_UUID);
    ...
    void testWithdrawMember_givenValidParameters_willReturnResponse() {
        // given
        willDoNothing().given(memberController).withdraw(testMemberWithdrawalRecord);

        // when
        ResponseEntity<DataResponse<Void>> responseEntity = memberRestController.withdrawMember(MEMBER_AUTH_BASIC_USER_AUTHORIZATION);
